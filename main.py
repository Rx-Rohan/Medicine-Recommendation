...
symptom_aliases = {
    # Fever
    "high body temperature": "high_fever",
...
def normalize_symptom(symptom):
    """Convert natural language symptom to model-compatible key"""
    symptom = symptom.strip().lower()
    # Direct match in aliases
    if symptom in symptom_aliases:
        return symptom_aliases[symptom]
    # Already valid symptom key
    symptom_underscore = symptom.replace(" ", "_")
    if symptom_underscore in symptoms_dict:
        return symptom_underscore
    # Partial match
    for alias, key in symptom_aliases.items():
        if alias in symptom or symptom in alias:
            return key
    return symptom_underscore  # return as-is and let it fail gracefully
...
def home():
    if request.method == 'POST':
        ...
        if symptoms =="Symptoms":
            ...
            # Normalize natural language to valid symptom keys
            user_symptoms = [normalize_symptom(s) for s in user_symptoms]
            ...
            try:
                ...
                message = "Symptom not recognized. Try: high_fever, headache, nausea, vomiting, fatigue, cough, skin_rash"
                return render_template('index.html', message=message)
